...
Imports IT.ELUX.BE
Imports IT.ELUX.BL
Public Class FormMantDetLetras
    ...
    Private Sub btnagregar_Click(sender As Object, e As EventArgs) Handles btnagregar.Click
        ...
        If gContador = 1 Then
            ...
            If npdcantidad.Value > 0 Then
                ...
                If gContador = 1 Then
                    ...
                    If FormMantLetras.dgvt_doc.Rows.Count > 0 Then
                        For i = 0 To FormMantLetras.dgvt_doc.Rows.Count - 1
                            If FormMantLetras.dgvt_doc.Rows(i).Cells("ART_COD").Value = cmbart.SelectedValue Then
                    ...
                    If icontador > 0 Then
                        FormMantLetras.dgvt_doc.Rows.Add(FormMantLetras.txtt_doc.Text, '0
                                                              FormMantLetras.cmb_serdoc.Text,'1
                                                              FormMantLetras.txtnumero.Text, '2
                                                              FormMantLetras.txtt_doc.Text, '3
                                                              FormMantLetras.cmb_serdoc.Text,'4
                                                              FormMantLetras.txtnumero.Text & "-" & icontador, '5
                                                              FormMantLetras.txtctct_cod.Text,'6 
                                                              npdcantidad.Value, '7
                                                              cmbart.SelectedValue,'8
                                                              sValor, '9
                                                              medida, '10
                                                              cmbuni.SelectedValue,'11
                                                              dtpfec_ent.Value, "", "", "", "", "+", txtobservacion.Text, '18
                                                              "", '19
                                                              ...
                                                              txtigv_dimpor.Text, FormMantLetras.txtmon.Text,  '28
                                                              FormMantLetras.dtpfecha.Text, '29
                                                              gsUser, '30
                                                              ...
                                                              FormMantLetras.cmbestado.Text) '45
                        Dispose()
                    Else
                        FormMantLetras.dgvt_doc.Rows.Add(FormMantLetras.txtt_doc.Text, '0
                                                              FormMantLetras.cmb_serdoc.Text,'1
                                                              ...
                                                              "0", "", txtdscto.Text, txtdscto_impor.Text, '43
                                                              txtdscto_dimpor.Text,'44
                                                              FormMantLetras.cmbestado.Text) '43
                        Dispose()
                    End If
                End If
            Else
        ...
        ElseIf gContador = 0 Then
            If FormMantLetras.txtmon.Text = "01" Then
            ...
            Dim sValor As String
            sValor = ARTICULOBL.SelectArt2(cmbart.SelectedValue)
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("CANTIDAD").Value = npdcantidad.Value '7
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("NOM_ART").Value = sValor
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("ART_COD").Value = cmbart.SelectedValue
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("OBSERVA").Value = txtobservacion.Text '16
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("TPRECIO_VENTA").Value = txttprecio_venta.Text
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("TPRECIO_DVENTA").Value = txttprecio_dventa.Text '19
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("IGV").Value = txtigv.Text
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("IGV_IMPOR").Value = txtigvimpor.Text
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("T_CAMB").Value = npdtcamb.Value
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("UPRECIO_VENTA").Value = npduprecio_venta.Value '23
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("UPRECIO_DVENTA").Value = npduprecio_dventa.Value '24
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("IGV_DIMPOR").Value = txtigv_dimpor.Text
            FormMantLetras.dgvt_doc.Rows(FormMantLetras.dgvt_doc.CurrentRow.Index).Cells("UNIDAD").Value = cmbuni.SelectedValue '29
            Dispose()
        End If
    End Sub
        ...
        If FormMantLetras.txtmon.Text = "01" Then
            ...
            If npdcantidad.Value > 0 And npduprecio_dventa.Value > 0 Then
                If txtigv.Text > 0 Then
                    ...
                    txttprecio_venta.Text = Math.Round(npduprecio_dventa.Text * npdcantidad.Text * npdtcamb.Text, 2)
                    txttprecio_dventa.Text = Math.Round(npduprecio_dventa.Text * npdcantidad.Text, 2)
                    txtigvimpor.Text = Math.Round(txttcompras.Text - txttprecio_venta.Text)
                    txtigv_dimpor.Text = Math.Round(txttcomprad.Text - txttprecio_dventa.Text)
                    'txttcomprad.Text = Format(CType(txttcomprad.Text, Decimal), "###,##0.00")
                    'txttcompras.Text = Format(CType(txttcompras.Text, Decimal), "###,##0.00")
                    'txtigv_dimpor.Text = Format(CType(txtigv_dimpor.Text, Decimal), "###,##0.00")
                    'txtigvimpor.Text = Format(CType(txtigvimpor.Text, Decimal), "###,##0.00")
    ...
    Private Sub txtnro_docu1_LostFocus(sender As Object, e As EventArgs) Handles txtnro_docu1.LostFocus
