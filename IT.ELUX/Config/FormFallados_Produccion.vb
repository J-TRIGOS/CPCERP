...
Imports IT.ELUX.BE
Imports IT.ELUX.BL
Public Class FormFallados_Produccion
    Private ARTICULOBL As New ARTICULOBL
    Private ELFALLADOSBL As New ELFALLADOSBL
...
#End Region
    ...
    Private Function OkData() As Boolean
        ...
        ElseIf cmbcosto.SelectedIndex = -1 Then
            ...
            txtcosto.Focus()
            Return False
        Else 'dtpfec_gene.Value.Date > dtpfecent.Value.Date Then
            '    MsgBox("La fecha de Termino debe ser mayor a la fecha de inicio", MsgBoxStyle.Exclamation)
            '    dtpfecent.Focus()
            '    Return False
    ...
    Private Sub SaveData()
        ...
        If MessageBox.Show("Desea grabar el Registro",
            ...
            Exit Sub
        End If
        'Dim t As DateTime = dtphoratermino.Value.ToLongTimeString
        'Dim t1 As DateTime = dtphoragene.Value.ToLongTimeString
        'Dim dl As TimeSpan = t - t1
        Dim mes As String
        ...
        'If Convert.ToDouble(Mid(dl.ToString, 1, 2)) < 0 Then
        '    MsgBox("La hora de termino es mayor a la hora de inicio")
        '    dtphoragene.Select()
        'Else
        '    txtdifhora.Text = Mid(dl.ToString, 1, 8)
        'End If
        Dim nro As String
        If flagAccion = "N" Then
            nro = ELFALLADOSBL.SelectNro(txtt_doc.Text, Format(dtpfec_gene.Value, "yyyy"))
        ...
        Try
            Dim ELFALLADOSBE As New ELFALLADOSBE
            ...
            If sOp = "0" Then
                ...
                ELFALLADOSBE.T_DOC_REF = RTrim(txtt_doc.Text)
                If rdbnew.Checked Then
                    ' cmbaño.Text
                    If lblserorden1.Text = "" Then
                        ELFALLADOSBE.SER_DOC_REF = sSDoc
                        ELFALLADOSBE.SER_ORDEN = ""
                    Else
                        ELFALLADOSBE.SER_DOC_REF = txtserdoc.Text
                        ELFALLADOSBE.SER_ORDEN = cmbaño.Text
                    End If

                Else
                    ELFALLADOSBE.SER_DOC_REF = RTrim(txtserdoc.Text)
                    ELFALLADOSBE.SER_ORDEN = ""
                End If 'ELFALLADOSBE.SER_DOC_REF = RTrim(txtserdoc.Text)
                ELFALLADOSBE.NRO_DOC_REF = RTrim(txtnro.Text)
                ELFALLADOSBE.ART_COD = txtcodart.Text
                ELFALLADOSBE.FEC_GENE = dtpfec_gene.Value
                ELFALLADOSBE.FEC_TERMINO = RTrim(dtpfecent.Value)
                ELFALLADOSBE.ART_VENTA = RTrim(txtfardo.Text)
                ELFALLADOSBE.ART_VENTA1 = RTrim(txtart_venta1.Text)
                ELFALLADOSBE.ART_VENTA2 = RTrim(txtart_venta2.Text)
                ELFALLADOSBE.ART_VENTA3 = RTrim(txtfardo4.Text)
                ELFALLADOSBE.LOTE = RTrim(txtlote.Text)
                ELFALLADOSBE.LOTE1 = RTrim(txtlote1.Text)
                ELFALLADOSBE.LOTE2 = RTrim(txtlote2.Text)
                ELFALLADOSBE.LOTE3 = RTrim(txtlote3.Text)
                ELFALLADOSBE.LOTE4 = RTrim(txtlote4.Text)
                ELFALLADOSBE.LOTE = RTrim(txtlote.Text)
                ELFALLADOSBE.UNIDAD = cmbuni.SelectedValue
                ELFALLADOSBE.LINEA = cmbc_costo.SelectedValue
                ELFALLADOSBE.NRO_ORDEN = RTrim(txtnroprod.Text)
                ELFALLADOSBE.CCO_COD = txtcosto.Text
                If rdbfinal.Checked Then
                    ELFALLADOSBE.X_ENTREGA = "F"
                ElseIf rdbparcial.Checked Then
                    ELFALLADOSBE.X_ENTREGA = "P"
                End If
                ELFALLADOSBE.CANTIDAD = npdcantidad.Value
                ELFALLADOSBE.EST = "H"

                ELFALLADOSBE.EST2 = "0"

                ELFALLADOSBE.OBSERVA2 = RTrim(txtobserva2.Text)
                ELFALLADOSBE.LINEA = RTrim(txtc_costo.Text)
                ELFALLADOSBE.FEC_DIA1 = txtfec_dia.Text

                If cmbturno.SelectedIndex = 0 Then
                    ELFALLADOSBE.TURNO = "A"
                Else
                    ELFALLADOSBE.TURNO = "B"
                End If
                'Dim dtphg As String = Format(dtphoragene.Value, "HH:mm:ss")
                'DateTime.Now.ToString("hh:mm:ss")
                ELFALLADOSBE.HORA_GENE = dtphoragene.Value
                'dtphg = Format(dtphoratermino.Value, "HH:mm:ss")
                ELFALLADOSBE.HORA_TERMINO = dtphoratermino.Value
                ELFALLADOSBE.DIF_HORA = txtdifhora.Text

                'Dim TIME As TimeSpan = dtphoratermino.Value - dtphoragene.Value
                ...
                'If Convert.ToDouble(Mid(MI.ToString, 1, 2)) < 0 Then
                '    MsgBox("La hora de termino es mayor a la hora de inicio")
                '    dtphoragene.Select()
                'Else
                ...
                'difhora = Mid(dl.ToString, 1, 8)
                'difhora = DateDiff(DateInterval.Minute, dtphoragene.Value, dtphoratermino.Value)
                ELFALLADOSBE.UND_H = Math.Round(npdcantidad.Value / difhora, 2)
                ELFALLADOSBE.NUM_DIF = difhora
                ELMVLOGSBE.log_codusu = gsCodUsr
                If sOp = "0" Then
                    ELFALLADOSBE.USUARIO_RP = RTrim(gsUser)
                    ELFALLADOSBE.EST1 = "0"
                ElseIf sOp = "1" Then
                    ELFALLADOSBE.USUARIO_OB = RTrim(gsUser)
                    'Observado
                    ELFALLADOSBE.EST1 = "1"
                End If
                If ELFALLADOSBE.EST1 = "1" Then
                    ELFALLADOSBE.EST1 = "0"
                End If
                ELFALLADOSBE.nrodia = lblnrodia.Text
                ELFALLADOSBE.anho = lblanho.Text
                If rdbnew.Checked Then
                    ELFALLADOSBE.op = "0"
                End If
                gsError = ELFALLADOSBL.SaveRow(ELFALLADOSBE, DETALLE_DOCUMENTOBE, ELMVLOGSBE, flagAccion, txtserdoc.Text)
            ElseIf sOp = "1" Then
                ELFALLADOSBE.T_DOC_REF = RTrim(txtt_doc.Text)
                ELFALLADOSBE.SER_DOC_REF = RTrim(txtserdoc.Text)
                ELFALLADOSBE.NRO_DOC_REF = RTrim(txtnro.Text)
                ELFALLADOSBE.OBSERVA1 = RTrim(txtobserva1.Text)
                ELMVLOGSBE.log_codusu = gsCodUsr
                ELFALLADOSBE.USUARIO_OB = RTrim(gsUser)
                ELFALLADOSBE.EST1 = "1"
                gsError = ELFALLADOSBL.SaveRow(ELFALLADOSBE, DETALLE_DOCUMENTOBE, ELMVLOGSBE, "M1", txtserdoc.Text)
            End If
    ...
    Private Sub tsbForm_ItemClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tsbForm.ItemClicked
        ...
        Select Case sFunc
            ...
            Case "anular"
                ...
                ELFALLADOSBE.T_DOC_REF = txtt_doc.Text
                ELFALLADOSBE.SER_DOC_REF = txtserdoc.Text
                ELFALLADOSBE.NRO_DOC_REF = txtnro.Text
                Dim ELMVLOGSBE As New ELMVLOGSBE
                ELMVLOGSBE.log_codusu = gsCodUsr
                gsError = ELFALLADOSBL.SaveRow(ELFALLADOSBE, DETALLE_DOCUMENTOBE, ELMVLOGSBE, "A", txtserdoc.Text)
    ...
    Private Sub GetData(ByVal sT_Ref As String, ByVal sS_Ref As String, ByVal sN_Ref As String)
        ...
        If sTDoc <> "RFAL" Then
            dt = ELFALLADOSBL.SelectRow(sT_Ref, sS_Ref, sN_Ref)
    ...
    Private Sub FormReporte_Produccion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        bPrimero = True
        gsError = ""
        Me.Text = "Fallados Producción"
        'Cargar los combos
        Dim dt As DataTable = ARTICULOBL.SelectDescripcion1()
            ...
            Case 1
                If txtfec_dia.Text <= "03/12/2020" Then
                    ...
                    If Me.txtt_doc.Text <> "RFAL" Then
                        ...
                        Next
                    Else
                        dt = ELFALLADOSBL.SelectRow1(Trim(sTDoc), Trim(sSDoc), Trim(sNDoc))
                        For Each Registro In dt.Rows
    ...
    Private Sub cmblinea_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmblinea.SelectedIndexChanged
        ...
        If cmblinea.SelectedIndex <> -1 Then
            Dim dt As DataTable = ARTICULOBL.SelectDescripcion3(sCode)
            GetCmb("lin_codigo", "lin_descri", dt, cmbsublinea)
        End If
        bSegundo = False
    End Sub
    ...
    Private Sub btnbuscar_Click(sender As Object, e As EventArgs) Handles btnbuscar.Click
        ...
        If gLinea <> Nothing And gSubLinea <> Nothing And gArt <> Nothing Then
            If gLinea >= 400 Then
                MsgBox("Valor fuera de los limites", MsgBoxStyle.Information)
            Else
    ...
    Private Sub dtphoratermino_LostFocus(sender As Object, e As EventArgs) Handles dtphoratermino.LostFocus
        ...
        'Dim TIME As TimeSpan
        'TIME = dtphoratermino.Value - dtphoragene.Value
        'If Convert.ToDouble(Mid(TIME.ToString, 1, 2)) < 0 Then
        '    MsgBox("La hora de termino es mayor a la hora de inicio")
        '    dtphoragene.Select()
        'Else
        '    txtdifhora.Text = Mid(TIME.ToString, 1, 8)
        'End If
        'txtdifhora.Text = Nothing
    ...
    Private Sub txtnroprod_LostFocus(sender As Object, e As EventArgs) Handles txtnroprod.LostFocus
        ...
        If rdbnew.Checked Then
            ...
            If txtc_costo.Enabled = False Then
                'Dim dt As DataTable
                'lblserorden.Text = ELPRODUCCIONBL.SelProdNro(txtnroprod.Text)
                'If lblserorden.Text <> "" Then
                ...
                If txtc_costo.Enabled = False Then
                    ...
                    For Each Registro In dt.Rows
                        ...
                        'xtArt.Text = IIf(IsDBNull(Registro("ART_COD")), "", Registro("ART_COD"))
                        'xtnomart.Text = ARTICULOBL.SelectArt2(txtArt.Text)
    ...
    Private Sub cmbturno_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbturno.SelectedIndexChanged
        ...
        If (anho Mod 4 = 0 And anho Mod 100 <> 0 Or anho Mod 400 = 0) Then
            lblnrodia.Text = ELFALLADOSBL.SelectNrodia("1", mes, dia) '.PadLeft(4, "0")
        Else
            lblnrodia.Text = ELFALLADOSBL.SelectNrodia("0", mes, dia) '.PadLeft(4, "0")            
        End If
