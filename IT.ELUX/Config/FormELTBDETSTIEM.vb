...
Public Class FormELTBDETSTIEM
    ...
    Private Function GetCmb(ByVal sCodigo As String, ByVal sDescri As String, ByVal dtSelect As DataTable, ByVal cmb As ComboBox) As DataTable
        ...
        cmb.SelectedIndex = -1
    End Function
    Private Sub FormELTBDETSTIEM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Dim dt As DataTable
        ...
        '-- ------------------------- --
        If cmbactividad.Visible = True Then
            cmbactividad.Text = txtact.Text
        End If
    End Sub

    Private Sub FormELTBDETSTIEM_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        Dispose()
    End Sub
    ...
    Private Sub btnaceptar_Click(sender As Object, e As EventArgs) Handles btnaceptar.Click
        ...
        Dim MI As Integer = 0
        Dim SE As Integer = 0
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("FEC_INICIO").Value = dtpfec_inicio.Value
        ...
        If HO > "17" Then
            ...
            Exit Sub
        End If
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("Hora_Inicio").Value = dtphoragene.Text
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("Hora_Final").Value = dtphoratermino.Text
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("FEC_INICIO").Value = Mid(dtpfec_inicio.Value, 1, 10)
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("FEC_TERMINO").Value = Mid(dtpfec_termino.Value, 1, 10)
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("Num_Hora").Value = HO.ToString.PadLeft(2, "0") & ":" & MI.ToString.PadLeft(2, "0") & ":" & SE.ToString.PadLeft(2, "0")
        FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("Dif_Hora").Value = DateDiff(DateInterval.Minute, dtphoragene.Value, dtphoratermino.Value)
        If cmbactividad.Visible = True Then
            FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("ACT_REALIZAR").Value = cmbactividad.Text
        Else
            FormELTBSTiem.dgvtiemper.Rows(FormELTBSTiem.dgvtiemper.CurrentRow.Index).Cells("ACT_REALIZAR").Value = txtact.Text
        End If
        FormELTBSTiem.dgvtiemper.Refresh()
        '    End If
        'Next
        Dispose()
    End Sub
