...
#End Region
    ...
    Private Sub tsbForm_ItemClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles tsbForm.ItemClicked
        ...
        Select Case sFunc
            ...
            Case "certi_calidad"
                ...
                gsPathRpt = gsIpserver & "sistema\E.LUX\REPORTES\02\RPT02_SP_DOCUMENTO_CERTIFICADO_CALIDAD.rpt"
                gsRptPath = gsPathRpt
            ...
            Case "acta"
                ...
                gsRptArgs(1) = RTrim(cmb_serdoc.Text)
                gsRptArgs(2) = RTrim(txtnumero.Text)
                gsPathRpt = gsIpserver & "sistema\E.LUX\REPORTES\02\RPT02_ACTA_ENTREGA_GD.rpt"
                gsRptPath = gsPathRpt
                FormReportes.ShowDialog()
                Exit Sub
            Case "guia_transpor"
                ...
                gsPathRpt = gsIpserver & "sistema\E.LUX\REPORTES\02\RPT02_SP_DOCUMENTO_GUIA_TRASNPORTISTA.rpt"
                gsRptPath = gsPathRpt
                FormReportes.ShowDialog()
                Exit Sub
            Case "save"
            ...
            Case "Print"
                If txtt_movinv.Text <> "S06" Then
                    ...
                    gsPathRpt = gsIpserver & "sistema\E.LUX\REPORTES\02\RPT02_GUIADESPACHO.rpt"
                    gsRptPath = gsPathRpt
                    FormReportes.ShowDialog()
                    Exit Sub
                Else
                    ...
                    gsRptArgs(2) = RTrim(txtnumero.Text)
                    gsRptArgs(3) = RTrim(txtt_movinv.Text)
                    gsPathRpt = gsIpserver & "sistema\E.LUX\REPORTES\02\RPT02_GUIADESPACHO_EXP.rpt"
                    gsRptPath = gsPathRpt
                    FormReportes.ShowDialog()
                    Exit Sub
                End If
    ...
    Private Sub FormMantGuiaDespacho_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dgvt_doc.Columns.Add("TRANSP_COD", "Codigo") '43
        dgvt_doc.Columns.Add("CHOFER", "Nombre del Chofer") '44
        dgvt_doc.Columns.Add("BREVETE", "Brevete") '45
        dgvt_doc.Columns.Add("PLACA", "Placa") '46
        dgvt_doc.Columns.Add("CONFIGURACION", "Configuracion") '47
        dgvt_doc.Columns.Add("CERTIFICADO", "Certificado") '48
        dgvt_doc.Columns.Add("TIPO_UNIDAD", "tipo Unidad") '49
        dgvt_doc.Columns.Add("COLOR", "Serie Transportista") '50
        dgvt_doc.Columns.Add("NRO_DOCU2", "Nro transportista") '51
        dgvt_doc.Columns.Add("FEC_ENT", "FEC.ENT") '52
        dgvt_doc.Columns.Add("OBSERVA", "Observa") '53
        dgvt_doc.Columns.Add("PESO", "PESO") '54
            ...
            Case 1
                ...
                For Each row As DataRow In dtArticulo.Rows
                    ...
                    dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")),'0
                                      ...
                                      IIf(IsDBNull(row("TRANSP_COD")), "", row("TRANSP_COD")),'43
                                      IIf(IsDBNull(row("CHOFER")), "", row("CHOFER")),'44
                                      IIf(IsDBNull(row("BREVETE")), "", row("BREVETE")),'45
                                      IIf(IsDBNull(row("PLACA")), "", row("PLACA")),'46
                                      IIf(IsDBNull(row("CONFIGURACION")), "", row("CONFIGURACION")),'47
                                      IIf(IsDBNull(row("CERTIFICADO")), "", row("CERTIFICADO")), '48
                                      IIf(IsDBNull(row("TIPO_UNIDAD")), "", row("TIPO_UNIDAD")), '49
                                      IIf(IsDBNull(row("COLOR")), "", row("COLOR")),'50
                                      IIf(IsDBNull(row("NRO_DOCU2")), "", row("NRO_DOCU2")),'51
                                      IIf(IsDBNull(row("FEC_ENT")), "", row("FEC_ENT")),'52
                                      IIf(IsDBNull(row("OBSERVA")), "", row("OBSERVA")),
                                      IIf(IsDBNull(row("PESO")), "", row("PESO"))) '53
