...
Imports IT.ELUX.BE
Imports IT.ELUX.BL
Public Class FormMantProvisiones
...
#End Region
    ...
    Private Function OkData() As Boolean
        ...
        If gsMes >= "202209" Then
            If txt_actFlujo.Text = "" Then
                MsgBox("Ingrese Cod. Act. FLujo", MsgBoxStyle.Exclamation)
                txt_actFlujo.Focus()
                Return False
            End If
        End If
        ...
        If gsMes >= "202209" Then
            If txt_ActCaja.Text = "" Then
                MsgBox("Ingrese Cod. Act. Caja", MsgBoxStyle.Exclamation)
                txt_ActCaja.Focus()
                Return False
            End If
        End If
        ...
        If txtt_pago.Text = "" Then
            MsgBox("Ingrese Forma de Pago", MsgBoxStyle.Exclamation)
            txtt_pago.Focus()
            Return False
        End If
        ...
        If txtnumero.Text = "" Then
            ...
            Return False
        End If
        If FormMain.chkest.Checked = False And flagAccion = "M" Then
    ...
    Private Sub SaveData()
        ...
        Dim m1 As String = dtpfechaprov.Value.Month
        Dim m As String
        ...
        Next
        PROVISIONESBE.T_DOC_REF = RTrim(txtt_doc.Text)
        PROVISIONESBE.TDOC = lbl_tdoc.Text
        PROVISIONESBE.SDOC = lbl_sdoc.Text
        PROVISIONESBE.NDOC = lbl_ndoc.Text
        PROVISIONESBE.FEC = lbl_fec.Text
        PROVISIONESBE.FECP = bFecP 'Mid(txtmskfecgene.Text, 7, 4) & Mid(txtmskfecgene.Text, 4, 2).ToString.PadLeft(2, "0") & Mid(txtmskfecgene.Text, 1, 2).ToString.PadLeft(2, "0") 'dtpfecha.Value.Year.ToString & dtpfecha.Value.Month.ToString.PadLeft(2, "0") & dtpfecha.Value.Day.ToString.PadLeft(2, "0")
        PROVISIONESBE.P = lbl_prov.Text
        PROVISIONESBE.SER_DOC_REF = RTrim(txtser_doc_ref.Text)
        PROVISIONESBE.NRO_DOC_REF = RTrim(txtnumero.Text)
        PROVISIONESBE.T_OPE = txtope.Text
        If cmbarticulo.SelectedIndex = 1 Then
            PROVISIONESBE.ART_COD = "07010001"
        ElseIf cmbarticulo.SelectedIndex = 0 Then
            PROVISIONESBE.ART_COD = "05990001"
        Else
            PROVISIONESBE.ART_COD = ""
        End If
        ...
        End If
        dt = REQUERIMIENTOBL.getT_CAMB(Mid(txtmskfecgene.Text, 7, 4) & "/" & mesfecha & "/" & mesdia)
        For Each Registro In dt.Rows
            cmb = IIf(IsDBNull(Registro("VENTA")), "", Registro("VENTA"))
        Next
        If txtt_doc.Text <> "07" And txtmoneda.Text <> "01" Then
            For i = 0 To dgvt_doc.Rows.Count - 1
                dgvt_doc.Rows(i).Cells("T_CAMB").Value = cmb
            Next
        End If
        If chkmtc.Checked Then
            For i = 0 To dgvt_doc.Rows.Count - 1
                dgvt_doc.Rows(i).Cells("t_camb").Value = npdtcamb.Value
            Next
        End If
        bTdoc = lbl_tdoc.Text
        bSdoc = lbl_sdoc.Text
        bNdoc = lbl_ndoc.Text
        bP = lbl_prov.Text
        PROVISIONESBE.FEC_PROV = txtmskfecprov.Text 'dtpfechaprov.Value
        PROVISIONESBE.SIGNO = "-"
        For i = 0 To dgvt_doc.Rows.Count - 1
        ...
        Next
        PROVISIONESBE.TPRECIO_COMPRA = DAcumula
        PROVISIONESBE.TPRECIO_DCOMPRA = DAcumula1
        PROVISIONESBE.T_DCTO = DAcumula2
        PROVISIONESBE.T_DCTO_DOLAR = DAcumula3
        PROVISIONESBE.T_IGV = DAcumula4
        ...
        PROVISIONESBE.FEC_GENE = txtmskfecgene.Text
        ...
        If cmb_x_ret.SelectedIndex = 1 Then
            PROVISIONESBE.X_RET = "N"
        ElseIf cmb_x_ret.SelectedIndex = 2 Then
            PROVISIONESBE.X_RET = "R"
        End If
        ...
        ElseIf cmbest.SelectedIndex = 3 Then
            PROVISIONESBE.EST = "3"
        End If
        PROVISIONESBE.REG_NRO = txtreg_nro.Text
        PROVISIONESBE.TIPO = "C"
        PROVISIONESBE.TIPO2 = txttipo2.Text
        If chktipo1.Checked Then
            PROVISIONESBE.TIPO1 = "1"
        End If
        If cmbest1.SelectedIndex = 1 Then
            PROVISIONESBE.EST1 = "D"
        ElseIf cmbest1.SelectedIndex = 2 Then
            PROVISIONESBE.EST1 = "P"
        ElseIf cmbest1.SelectedIndex = 3 Then
            PROVISIONESBE.EST1 = "R"
        End If
        PROVISIONESBE.NRO_PERCEPCION = txtnro_percepcion.Text
        PROVISIONESBE.DETRACCION = txtnro_detrac.Text
        'If dtpfec_det.Checked Then
        If txtnro_detrac.TextLength > 0 Then
            PROVISIONESBE.FEC_DET = txtmskfecdet.Text
        End If
        'If dtpfec_letra.Checked Then
        If txtletra.TextLength > 0 Then
            PROVISIONESBE.FEC_LETRA = txtmskfecletra.Text
        End If
        If chkmtc.Checked Then
            PROVISIONESBE.NUMPEDIDO = "1"
        End If

        PROVISIONESBE.LETRA = txtletra.Text
        mesaño = Replace(Mid(txtmskfecprov.Text, 4, 7), "/", "")
        m = Mid(txtmskfecprov.Text, 4, 2)
        PROVISIONESBE.MONTO_PAGADO = IIf(IsDBNull(dgvt_doc.Rows(0).Cells("MONTO_PAGADO").Value), 0, dgvt_doc.Rows(0).Cells("MONTO_PAGADO").Value)
        PROVISIONESBE.PROGRAMA = dgvt_doc.Rows(0).Cells("PROGRAMA").Value
        PROVISIONESBE.PORCENTAJE = Val(IIf(IsDBNull(dgvt_doc.Rows(0).Cells("PORCENTAJE").Value), 0, dgvt_doc.Rows(0).Cells("PORCENTAJE").Value))
        PROVISIONESBE.FARDO = dgvt_doc.Rows(0).Cells("FARDO").Value
        PROVISIONESBE.CTA_CBCO = dgvt_doc.Rows(0).Cells("CTA_CBCO").Value
        PROVISIONESBE.NOM_CTCT = txtnomproveedor.Text

        PROVISIONESBE.TIPO7 = txt_actFlujo.Text
        PROVISIONESBE.EST_MERCADERIA = txt_ActCaja.Text

        pagoFlujo = PROVISIONESBE.TIPO7
        pagoCaja = PROVISIONESBE.EST_MERCADERIA
        ...
        If gsUser <> "CHOYOS" And gsUser <> "SISTEMA" And gsUser <> "JHUAYLLACAYAN" Then
            ...
            Else
                If sMes1 = "12" And Mid(txtmskfecprov.Text, 7, 4) <= DateTime.Now.ToString("yyyy") - 1 Then
                    'If Mid(txtmskfecgene.Text, 1, 2) - DateTime.Now.ToString("dd") >= 20 Then
                    MsgBox("Mes Cerrado")
                    Exit Sub
                    'End If
                Else
        ...
        If gsError = "OK" Then
            ...
            If dgvtletras.Rows.Count > 0 Then
                ...
                If gsError3 = "OK" Then
                    ...
                    gsError2 = PROVISIONESBL.AsientoPR("Asiento", mesaño, m, RTrim(txtnumero.Text), txtser_doc_ref.Text, txtt_doc.Text, gsUser, txtproveedor.Text, dgvt_doc, pagoFlujo, pagoCaja)
                    If gsError2 = "OK" Then
                ...
                Else
                    ...
                    MsgBox("Error al Grabar Letras", MsgBoxStyle.Critical)
                End If
            Else
                gsError2 = PROVISIONESBL.AsientoPR("Asiento", mesaño, m, RTrim(txtnumero.Text), txtser_doc_ref.Text, txtt_doc.Text, gsUser, txtproveedor.Text, dgvt_doc, pagoFlujo, pagoCaja)
                If gsError2 = "OK" Then
            ...
            Try
                sFecCom = Mid(txtmskfecprov.Text, 7, 4) & Mid(txtmskfecprov.Text, 4, 2).ToString.PadLeft(2, "0") 'dtpfechaprov.Value.Year & dtpfechaprov.Value.Month.ToString.PadLeft(2, "0")
                dtUsuario = PROVISIONESBL.SelectRow(txtt_doc.Text, txtser_doc_ref.Text, txtnumero.Text, sFecCom, txtproveedor.Text)
        ...
        Try
            ...
            Select Case sFunc
                ...
                Case "Print"
                    ...
                    gsPathRpt = gsIpserver & "sistema\E.ELUX\REPORTES\02\RPT02_RPT_MOV_CT.rpt"
                    gsRptPath = gsPathRpt
    ...
    Private Sub GetData(ByVal sT_Ref As String, ByVal sS_Ref As String, ByVal sN_Ref As String)
        ...
        'MODIFICAR
        dtUsuario = PROVISIONESBL.SelectRow(sT_Ref, sS_Ref, sN_Ref, sFecCom, gsCode7)
        For Each Registro In dtUsuario.Rows
            ...
            End If
            If IIf(IsDBNull(Registro("NUMPEDIDO")), "", Registro("NUMPEDIDO")) = "1" Then
                npdtcamb.Enabled = True
                npdtcamb.ReadOnly = False
                chkmtc.Checked = True
            Else
                npdtcamb.Enabled = False
                npdtcamb.ReadOnly = True
            End If
    ...
    Private Sub FormMantProvisiones_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        If (FormMain.cmbaño.Text = "2021" And FormMain.cmbmes.SelectedIndex <= 8) Or
            (FormMain.cmbaño.Text <= "2020" And FormMain.cmbmes.SelectedIndex <= 11) Then
            If gsUser <> "CHOYOS" And gsUser <> "SISTEMA" Then
        ...
        Dim dt As DataTable = PROVISIONESBL.SelectT_DOC()
        ...
        dgvt_doc.Columns.Add("X_D", "Rep.") '40
        dgvt_doc.Columns.Add("NRO_DOCU2", "NRO_DOCU2") '41
        dgvt_doc.Columns.Add("CONFIGURACION", "SER_DOCU2") '42
        dgvt_doc.Columns.Add("TEMPLE", "T_DOCU2") '43
        dgvt_doc.Columns.Add("MONTO_PAGADO", "Monto Pagar S.") '44
        dgvt_doc.Columns.Add("PORCENTAJE", "Porc.") '45
        dgvt_doc.Columns.Add("CTA_CBCO", "CTA_CBCO") '46
        dgvt_doc.Columns.Add("FARDO", "Cod. Bien o Servicio") '47
        dgvt_doc.Columns.Add("PROGRAMA", "Cod. Ope") '48
        dgvt_doc.Columns.Add("PESO", "Peso") '49
        dgvt_doc.Columns.Add("USUACT", "UsuAct") '50
        ...
        Select Case gnOpcion
            ...
            Case 1
                ...
                'habilitar(True)
                dtArticulo = PROVISIONESBL.getListdgv(sTDoc, sSDoc, sNDoc, sFecCom, gsCode7)
                For Each row As DataRow In dtArticulo.Rows
                    dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")),'0
                                      ...
                                      IIf(IsDBNull(row("X_D")), "", row("X_D")),'40
                                      IIf(IsDBNull(row("NRO_DOCU2")), "", row("NRO_DOCU2")),'41
                                      IIf(IsDBNull(row("CONFIGURACION")), "", row("CONFIGURACION")),'42
                                      IIf(IsDBNull(row("TEMPLE")), "", row("TEMPLE")),'43
                                      IIf(IsDBNull(row("MONTO_PAGADO")), "", row("MONTO_PAGADO")),'44
                                      IIf(IsDBNull(row("PORCENTAJE")), "", row("PORCENTAJE")),'45
                                      IIf(IsDBNull(row("CTA_CBCO")), "", row("CTA_CBCO")),'46
                                      IIf(IsDBNull(row("FARDO")), "", row("FARDO")),'47
                                      IIf(IsDBNull(row("PROGRAMA")), "", row("PROGRAMA")),'48
                                      IIf(IsDBNull(row("PESO")), "", row("PESO")),
                                      IIf(IsDBNull(row("USUACT")), "", row("USUACT"))) '49
                    t_cmb = IIf(IsDBNull(row("T_CAMB")), "", row("T_CAMB"))
                    txt_usuCrea.Text = IIf(IsDBNull(row("USUARIO")), "", row("USUARIO"))
                    txt_usuAct.Text = IIf(IsDBNull(row("USUACT")), "", row("USUACT"))
                    txt_actFlujo.Text = IIf(IsDBNull(row("TIPO7")), "", row("TIPO7"))
                    txt_flujo.Text = IIf(IsDBNull(row("FLUJO")), "", row("FLUJO"))
                    txt_ActCaja.Text = IIf(IsDBNull(row("EST_MERCADERIA")), "", row("EST_MERCADERIA"))
                    txt_caja.Text = IIf(IsDBNull(row("CAJA")), "", row("CAJA"))
                Next
                npdtcamb.Value = t_cmb
                dtArticulo = PROVISIONESBL.getListdgv3(sTDoc, sSDoc, sNDoc)
                For Each row As DataRow In dtArticulo.Rows
                    dgvtletras.Rows.Add(IIf(IsDBNull(row("CODIGO")), "", row("CODIGO")),'0
                                      ...
                                      IIf(IsDBNull(row("MONTO")), "", row("MONTO"))) '7
                ...
                dgvt_doc.Columns(48).Visible = False
                dgvt_doc.Columns(49).Visible = False
                If gsUser <> "SISTEMA" Or gsUser <> "CHOYOS" Then
                    If Mid(txtmskfecgene.Text, 7, 4) <= "2021" Or Mid(txtmskfecprov.Text, 7, 4) <= "2021" Then
                        MsgBox("El Año se encuentra cerrado")
                        tsbGrabar.Enabled = False
                    End If
                End If
                'bMes1 = Mid(txtmskfecprov.Text, 4, 2)
                If gsUser = "MTORRES" Then
                    If sMes2 = "01" And Mid(txtmskfecprov.Text, 7, 4) = DateTime.Now.ToString("yyyy") - 1 Then
                        If DateTime.Now.ToString("dd") >= 20 Then
                            'If Mid(txtmskfecprov.Text, 4, 2) <> 3 Then
                            MsgBox("Mes Cerrado")
                            tsbGrabar.Enabled = False
                            ...
                            Exit Sub
                        End If
                        If Mid(txtmskfecprov.Text, 4, 2) < 12 Then
                            MsgBox("Mes Cerrado")
                            tsbGrabar.Enabled = False
                        End If
                    Else
    ...
    Private Sub btnmodificar_Click(sender As Object, e As EventArgs) Handles btnmodificar.Click
        Dim dt As DataTable
        If dgvt_doc.Rows.Count > 0 Then
            Dim frm As New FormMantDetProvisiones
            ...
            End If
            If txtt_doc.Text <> "07" And txtt_doc.Text <> "14" And txtt_doc.Text <> "36" Then
                frm.txtcco_nom.Text = CCOSTOBL.SelectNom(frm.txtcco_cod.Text)
            Else
                frm.txtcco_cod.ReadOnly = False
            End If
            ...
            If flagAccion = "M" Then
                If frm.npdtcamb.Value = 0 Then
                    ...
                    Else
                        mesdia = Mid(txtmskfecgene.Text, 1, 2)
                    End If
                    dt = PROVISIONESBL.getT_CAMB(Mid(txtmskfecgene.Text, 7, 4) & "/" & mesfecha & "/" & mesdia)
                    For Each Registro In dt.Rows
            ...
            '--f
            If chkmtc.Checked Then
                frm.npdtcamb.Enabled = True
                frm.npdtcamb.ReadOnly = False
                frm.npdtcamb.Value = npdtcamb.Value
            Else
                'frm.npdtcamb.Enabled = False
                frm.npdtcamb.ReadOnly = True
            End If
    ...
    Private Sub btnagregar_Click(sender As Object, e As EventArgs) Handles btnagregar.Click
        ...
        dt = PROVISIONESBL.getT_CAMB(sFecha)
        Dim frm As New FormMantDetProvisiones
        Dim mesfecha As String
        Dim mesdia As String
        ' If Mid(txtmskfecgene.Text, 4, 2) = "1" Then
        mesfecha = Mid(txtmskfecgene.Text, 4, 2).PadLeft(2, "0")
        'Else
        '    mesfecha = txtmskfecgene.Text
        'End If
        ' If Mid(txtmskfecgene.Text, 1, 2) = "1" Then
        mesdia = Mid(txtmskfecgene.Text, 1, 2).PadLeft(2, "0")
        'Else
        'mesdia = Mid(txtmskfecgene.Text, 1, 2)
        'End If
        dt = PROVISIONESBL.getT_CAMB(Mid(txtmskfecgene.Text, 7, 4) & "/" & mesfecha & "/" & mesdia)
    ...
    Private Sub dgvt_doc_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvt_doc.CellDoubleClick
        ...
        If txtmoneda.TextLength > 0 Then
            If dgvt_doc.Rows.Count > 0 Then
                ...
                frm.dgvt_docdet.Columns.Add("NRO_DOCU2", "NRO_DOCU2") '40
                frm.dgvt_docdet.Columns.Add("CONFIGURACION", "SER_DOCU2") '41
                Dim dtArticulo As DataGridView
                dtArticulo = dgvt_doc
                Dim tc As Double
                For Each row As DataGridViewRow In dtArticulo.Rows
    ...
    Private Sub btndocu_Click(sender As Object, e As EventArgs) Handles btndocu.Click
        Dim frm As New FormDocuProvCon
...
#Region "Txt y CMB"
    ...
    Private Sub dtpfechaprov_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpfechaprov.KeyDown
        If e.KeyValue = Keys.Enter Then
            dtpfecha.Focus()
        End If
    End Sub
    ...
    Private Sub dtpfec_letra_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpfec_letra.KeyDown
        If e.KeyValue = Keys.Enter Then
            btnagregar.Select()
        End If
    ...
    Private Sub txtmskfecgene_LostFocus(sender As Object, e As EventArgs) Handles txtmskfecgene.LostFocus
        Try
            ...
            Else
                mesfecha = Mid(txtmskfecgene.Text, 4, 2)
            End If
            If flagAccion = "M" And chkmtc.Checked Then
    ...
    Private Sub chkmtc_CheckedChanged(sender As Object, e As EventArgs) Handles chkmtc.CheckedChanged
        If chkmtc.Checked Then
            npdtcamb.Enabled = True
            npdtcamb.ReadOnly = False
        Else
    ...
    Private Sub txt_ActCaja_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_ActCaja.KeyDown
        ...
        If e.KeyValue = Keys.Enter Then
            Dim dtActFlujo As New DataTable
            dtActFlujo = ELDOCUMENTOBL.BuscarActFlujoCaja(txt_actFlujo.Text, txt_ActCaja.Text, "010")
            If dtActFlujo.Rows.Count > 0 Then
