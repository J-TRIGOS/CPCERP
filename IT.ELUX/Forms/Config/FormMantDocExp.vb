...
#End Region
    ...
    Private Sub SaveData()
        ...
        ELTBDOCEXPBE.TIPO7 = txt_actFlujo.Text
        ELTBDOCEXPBE.EST_MERCADERIA = txt_ActCaja.Text
        ...
        If gsError = "OK" Then
            ...
            gsError2 = ELTBDOCEXPBL.AsientoPR("Asiento", mesaño, m, RTrim(txtnumero.Text), txtser_doc_ref.Text, txtt_doc.Text, gsUser, txtproveedor.Text, dgvt_doc, txt_actFlujo.Text, txt_ActCaja.Text)
            If gsError2 = "OK" Then
    ...
    Private Sub GetData(ByVal sT_Ref As String, ByVal sS_Ref As String, ByVal sN_Ref As String)
        ...
        For Each Registro In dtUsuario.Rows
            ...
            npdadvalore.Value = Val(IIf(IsDBNull(Registro("ADVALOR")), 0, Registro("ADVALOR")))
    ...
    Private Sub FormMantDocExp_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dgvt_doc.Columns.Add("ADVALORE", "Advalore") '40
        dgvt_doc.Columns.Add("SREQ", "Ser. Req.") '41
        dgvt_doc.Columns.Add("NREQ", "Nro. Req.") '42
        dgvt_doc.Columns.Add("X_D", "Rep.") '43
            ...
            Case 1
                ...
                For Each row As DataRow In dtArticulo.Rows
                    dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")),'0
                                      ...
                                      IIf(IsDBNull(row("ADVALORE")), 0, row("ADVALORE")),'40
                                      IIf(IsDBNull(row("SDOC")), "", row("SDOC")),'41
                                      IIf(IsDBNull(row("NDOC")), "", row("NDOC")),'42
                                      IIf(IsDBNull(row("X_D")), "", row("X_D")),'43
    ...
    Private Sub btnmodificar_Click(sender As Object, e As EventArgs) Handles btnmodificar.Click
        ...
        If dgvt_doc.Rows.Count > 0 Then
            ...
            frm.npdadvalore.Text = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("ADVALORE").Value
            ...
            frm.dgvt_docdet.Columns.Add("ADVALORE", "ADVALORE") '40
            frm.dgvt_docdet.Columns.Add("X_D", "X_D") '41
            frm.dgvt_docdet.Columns.Add("NRO_DOCU2", "NRO_DOCU2") '42
            frm.dgvt_docdet.Columns.Add("CONFIGURACION", "SER_DOCU2") '43
            ...
            For Each row As DataGridViewRow In dtArticulo.Rows
                ...
                frm.dgvt_docdet.Rows.Add(IIf(IsDBNull(row.Cells("T_DOC_REF").Value), "", row.Cells("T_DOC_REF").Value),'0
                                          ...
                                          IIf(IsDBNull(row.Cells("CCO_COD").Value), "", row.Cells("CCO_COD").Value),'6
                                          IIf(IsDBNull(row.Cells("STATUS").Value), "", row.Cells("STATUS").Value),'7
                                          IIf(IsDBNull(row.Cells("CUENTA").Value), "", row.Cells("CUENTA").Value),'8
                                          IIf(IsDBNull(row.Cells("CUENTA_DEST").Value), "", row.Cells("CUENTA_DEST").Value),'9
                                          IIf(IsDBNull(row.Cells("SIGNO").Value), "", row.Cells("SIGNO").Value),'10
                                          IIf(IsDBNull(row.Cells("CTCT_COD").Value), "", row.Cells("CTCT_COD").Value),'11
                                          IIf(IsDBNull(row.Cells("CANTIDAD").Value), "", row.Cells("CANTIDAD").Value),'12
                                          IIf(IsDBNull(row.Cells("ART_COD").Value), "", row.Cells("ART_COD").Value),'13
                                          IIf(IsDBNull(row.Cells("UNIDAD").Value), "", row.Cells("UNIDAD").Value), '14
                                          IIf(IsDBNull(row.Cells("TPRECIO_COMPRA").Value), "", row.Cells("TPRECIO_COMPRA").Value),'15
                                          IIf(IsDBNull(row.Cells("TPRECIO_DCOMPRA").Value), "", row.Cells("TPRECIO_DCOMPRA").Value),'16
                                          IIf(IsDBNull(row.Cells("IGV").Value), "", row.Cells("IGV").Value),'17
                                          IIf(IsDBNull(row.Cells("IGV_IMPOR").Value), "", row.Cells("IGV_IMPOR").Value),'18
                                          tc,'19
                                          IIf(IsDBNull(row.Cells("UPRECIO_COMPRA").Value), "", row.Cells("UPRECIO_COMPRA").Value),'20
                                          IIf(IsDBNull(row.Cells("UPRECIO_DCOMPRA").Value), "", row.Cells("UPRECIO_DCOMPRA").Value),'21
                                          IIf(IsDBNull(row.Cells("IGV_DIMPOR").Value), "", row.Cells("IGV_DIMPOR").Value),'22
                                          IIf(IsDBNull(row.Cells("MONEDA").Value), "", row.Cells("MONEDA").Value),'23
                                          IIf(IsDBNull(row.Cells("FEC_GENE").Value), "", row.Cells("FEC_GENE").Value),'24
                                          IIf(IsDBNull(row.Cells("USUARIO").Value), "", row.Cells("USUARIO").Value),'25
                                          IIf(IsDBNull(row.Cells("FEC_DIA").Value), "", row.Cells("FEC_DIA").Value),'26
                                          IIf(IsDBNull(row.Cells("PROVEEDOR").Value), "", row.Cells("PROVEEDOR").Value),'27
                                          IIf(IsDBNull(row.Cells("DSCTO_IMPOR").Value), "", row.Cells("DSCTO_IMPOR").Value),'28
                                          IIf(IsDBNull(row.Cells("DSCTO_DIMPOR").Value), "", row.Cells("DSCTO_DIMPOR").Value),'29
                                          IIf(IsDBNull(row.Cells("DSCTO").Value), "", row.Cells("DSCTO").Value),'30
                                          IIf(IsDBNull(row.Cells("IES").Value), "", row.Cells("IES").Value),'31
                                          IIf(IsDBNull(row.Cells("IES_IMPOR").Value), "", row.Cells("IES_IMPOR").Value),'32
                                          IIf(IsDBNull(row.Cells("IES_DIMPOR").Value), "", row.Cells("IES_DIMPOR").Value),'33
                                          IIf(IsDBNull(row.Cells("CTA").Value), "", row.Cells("CTA").Value),'34
                                          IIf(IsDBNull(row.Cells("CTA_IMPOR").Value), "", row.Cells("CTA_IMPOR").Value),'35
                                          IIf(IsDBNull(row.Cells("CTA_DIMPOR").Value), "", row.Cells("CTA_DIMPOR").Value),'36
                                          IIf(IsDBNull(row.Cells("UPRECIO_AFECTOS").Value), "", row.Cells("UPRECIO_AFECTOS").Value),'37
                                          IIf(IsDBNull(row.Cells("UPRECIO_AFECTOD").Value), "", row.Cells("UPRECIO_AFECTOD").Value),'38
                                          IIf(IsDBNull(row.Cells("ART_NOM").Value), "", row.Cells("ART_NOM").Value),'39
                                          IIf(IsDBNull(row.Cells("ADVALORE").Value), 0, row.Cells("ADVALORE").Value),'40
                                          IIf(IsDBNull(row.Cells("X_D").Value), "", row.Cells("X_D").Value)) '41
    ...
    Private Sub dgvt_doc_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvt_doc.CellDoubleClick
        ...
        If txtmoneda.TextLength > 0 Then
            If dgvt_doc.Rows.Count > 0 Then
                ...
                frm.dgvt_docdet.Columns.Add("ADVALORE", "ADVALORE") '39
                ...
                For Each row As DataGridViewRow In dtArticulo.Rows
                    ...
                    frm.dgvt_docdet.Rows.Add(IIf(IsDBNull(row.Cells("T_DOC_REF").Value), "", row.Cells("T_DOC_REF").Value),'0
                                          ...
                                          IIf(IsDBNull(row.Cells("ADVALORE").Value), "", row.Cells("ADVALORE").Value),'39
                                          IIf(IsDBNull(row.Cells("X_D").Value), "", row.Cells("X_D").Value)) '40
                Next
                'End With
                '--f
                gsCode5 = dgvt_doc.CurrentRow.Index
