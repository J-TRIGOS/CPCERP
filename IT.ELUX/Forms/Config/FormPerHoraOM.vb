...
#End Region
    Private Sub FormPerHoraOM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ElseIf sBusAlm = "3" Then
            ...
            If FormReporte_Trabajo.cmbTurno.SelectedIndex = 1 Then
                Dim TestNoche As Date = #01/01/2001 08:00:00 PM#
                dtphoragene.Value = TestNoche
                dtphoratermino.Value = TestNoche
            Else
                Dim TestDia As Date = #01/01/2001 08:00:00 AM#
                dtphoragene.Value = TestDia
                dtphoratermino.Value = TestDia
            End If
...
#Region "Fecha"
            ...
            date1 = dtpfec_inicio.Value.ToShortDateString
            ...
            Else
                If dgvbusqueda.Rows.Count > 0 Then
                    ...
                    '        If a = "" Then
                    HO = Math.Abs(DateDiff(DateInterval.Hour, date1, date2))
...
#End Region
            ...
            ser = Mid(dgvbusqueda.Rows(dgvbusqueda.CurrentRow.Index).Cells("FEC_GENE").Value, 7, 4)
            ...
            For Each Registro In dtOM.Rows
                FormReporte_Trabajo.dgvdiaper.Rows.Add("", IIf(IsDBNull(Registro("T_DOC_REF")), "", Registro("T_DOC_REF")),                                      '0-1
                                                           ...
                                                           txtobs.Text,                                                                                          '8
                                                           Mid(dtphoragene.Text, 1, 8),                                                                          '9
                                                           Mid(dtphoratermino.Text, 1, 8),                                                                       '10
                                                           (HO.ToString.PadLeft(2, "0") & ":" & MI.ToString.PadLeft(2, "0") & ": 00"),                            '11     & SE.ToString.PadLeft(2, "0"))
                                                           Mid(dtpfec_inicio.Value, 1, 10),                                                                        '12
                                                           Mid(dtpfec_termino.Value, 1, 10),                                                                     '13
...
#Region "Tecnicos OM"
    Private Sub dtpfec_termino_LostFocus(sender As Object, e As EventArgs) Handles dtpfec_termino.LostFocus
        ...
        '   If sBusAlm = "3" Then
        '       If FormReporte_Trabajo.cmbTurno.SelectedIndex = 1 Then
        '           Dim dtpini As DateTime = dtpfec_termino.Value.AddDays(+1).ToShortDateString
        '           Dim dtpini1 As DateTime = dtpfec_termino.Value.AddDays(-2).ToShortDateString
        '           Dim Today As DateTime = DateTime.Now.ToShortDateString
        '           If DateTime.Compare(dtpini, Today) <= 0 Then
        '               MsgBox("La fecha de termino no debe ser menor", MsgBoxStyle.Exclamation)
        '               dtpfec_termino.Select()
        '               Exit Sub
        '           End If
        '           If DateTime.Compare(Today, dtpini1) <= 0 Then
        '               MsgBox("La fecha de termino no debe ser mayor", MsgBoxStyle.Exclamation)
        '               dtpfec_termino.Select()
        '               Exit Sub
        '           End If
        '           If dtpfec_termino.Value < dtpfec_inicio.Value Then
        '               dtpfec_termino.Value = dtpfec_inicio.Value
        '           End If
        '       End If
        '   End If
        '
    ...
    Private Sub dtphoratermino_LostFocus(sender As Object, e As EventArgs) Handles dtphoratermino.LostFocus
        ...
        If sBusAlm = "3" Then
            If FormReporte_Trabajo.cmbTurno.SelectedIndex = 1 Then
                If vb.Right((dtphoratermino.Value).ToString, 4) = "p.m." Then
                    dtpfec_termino.Value = FormReporte_Trabajo.dtpfec_gene.Value
                ElseIf vb.Right(dtphoratermino.Value, 4) = "a.m." Then
                    dtpfec_termino.Value = FormReporte_Trabajo.dtpfec_gene.Value.Date.AddDays(1)
                End If
            End If
        End If
        ...
        Else
            ...
            HO = Math.Abs(DateDiff(DateInterval.Hour, date1, date2)) 'Math.Abs(DateDiff(DateInterval.Hour, dtphoragene.Value, dtphoratermino.Value))
            MI = DateDiff(DateInterval.Minute, date1, date2) 'DateDiff(DateInterval.Minute, dtphoragene.Value, dtphoratermino.Value)
            SE = DateDiff(DateInterval.Second, date1, date2) 'DateDiff(DateInterval.Second, dtphoragene.Value, dtphoratermino.Value)
            MI = MI Mod 60
            SE = SE Mod 60
            If HO > "18" Then
                MsgBox("La hora mayor a las horas permitidas cambiar fechas")
    ...
    Private Sub dtphoragene_LostFocus(sender As Object, e As EventArgs) Handles dtphoragene.LostFocus
        If sBusAlm = "3" Then
            'If FormReporte_Trabajo.Label10.Text = "N" Then
            If FormReporte_Trabajo.cmbTurno.SelectedIndex = 1 Then
                    If vb.Right((dtphoragene.Value).ToString, 4) = "p.m." Then
                        dtpfec_inicio.Value = FormReporte_Trabajo.dtpfec_gene.Value
                    ElseIf vb.Right(dtphoragene.Value, 4) = "a.m." Then
                        dtpfec_inicio.Value = FormReporte_Trabajo.dtpfec_gene.Value.Date.AddDays(1)
                        dtpfec_termino.Value = FormReporte_Trabajo.dtpfec_gene.Value.Date.AddDays(1)
                        dtphoratermino.Value = dtphoragene.Value
                    End If
                End If
                'ElseIf FormReporte_Trabajo.Label10.Text = "M" Then
                '
                'End If
            End If
    End Sub
...
#End Region
    ...
    Private Sub dtpfec_inicio_LostFocus(sender As Object, e As EventArgs) Handles dtpfec_inicio.LostFocus
        '   If sBusAlm = "3" Then
        '       If FormReporte_Trabajo.cmbTurno.SelectedIndex = 1 Then
        '           Dim dtpini As DateTime = dtpfec_inicio.Value.AddDays(+1).ToShortDateString
        '           Dim dtpini1 As DateTime = dtpfec_inicio.Value.AddDays(-2).ToShortDateString
        '           Dim Today As DateTime = DateTime.Now.ToShortDateString
        '           If DateTime.Compare(dtpini, Today) <= 0 Then
        '               MsgBox("La fecha de inicio no debe ser menor", MsgBoxStyle.Exclamation)
        '               dtpfec_inicio.Select()
        '               Exit Sub
        '           End If
        '           If DateTime.Compare(Today, dtpini1) <= 0 Then
        '               MsgBox("La fecha de inicio no debe ser mayor", MsgBoxStyle.Exclamation)
        '               dtpfec_inicio.Select()
        '               Exit Sub
        '           End If
        '           If dtpfec_termino.Value < dtpfec_inicio.Value Then
