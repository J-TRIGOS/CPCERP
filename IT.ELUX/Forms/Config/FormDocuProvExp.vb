...
Public Class FormDocuProvExp
                ...
                For i = 0 To lvbusqueda1.Items.Count - 1
                    If lvbusqueda1.Items(i).Checked = True Then
                        ...
                        For Each row As DataRow In dt.Rows
                            ...
                            End If
                            'If s = "AFECTO" Then
                            FormMantDocExp.dgvt_doc.Rows.Add(FormMantDocExp.txtt_doc.Text, FormMantDocExp.txtser_doc_ref.Text, FormMantDocExp.txtnumero.Text,
                                                     ...
                                                     IIf(IsDBNull(row("CUENTA_DEST")), "", row("CUENTA_DEST")), "+", "155729857-2-2022", IIf(IsDBNull(row("CANTIDAD")), "", row("CANTIDAD")),
                                                     ...
                                                     FormMantDocExp.txtmskfecgene.Text, gsUser, RTrim(Date.Now), IIf(IsDBNull(row("PROVEEDOR")), 0, row("PROVEEDOR")), 0, 0, 0,
                            ...
                            'End If
                            If i = 1 Then
                                FormMantDocExp.txtt_pago.Text = IIf(IsDBNull(row("F_PAGO_ENT")), "", row("F_PAGO_ENT"))
                                FormMantDocExp.cmbt_pago.SelectedValue = IIf(IsDBNull(row("F_PAGO_ENT")), "", row("F_PAGO_ENT"))
                            End If
                        Next
                    End If
                Next
                    ...
                    If lvbusqueda1.Items(i).Checked = True Then
                        ...
                        For Each row As DataRow In dt.Rows
                            ...
                            Else
                                s = "INAFECTO"
                            End If
                            FormMantDocExp.dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")), IIf(IsDBNull(row("SER_DOC_REF")), "", row("SER_DOC_REF")),
                                                     IIf(IsDBNull(row("NRO_DOC_REF")), "", row("NRO_DOC_REF")),
                                                     ...
                                                     IIf(IsDBNull(row("T_CAMB")), 0, row("T_CAMB")), IIf(IsDBNull(row("UPRECIO_COMPRA")), 0, row("UPRECIO_COMPRA")),
                                                     IIf(IsDBNull(row("UPRECIO_DCOMPRA")), 0, row("UPRECIO_DCOMPRA")), IIf(IsDBNull(row("IGV_DIMPOR")), 0, row("IGV_DIMPOR")), IIf(IsDBNull(row("MONEDA")), "", row("MONEDA")),
                                                     FormMantDocExp.txtmskfecgene.Text, gsUser, RTrim(Date.Now), IIf(IsDBNull(row("PROVEEDOR")), "", row("PROVEEDOR")), 0, 0, 0,
                                                     0, 0, 0, 0, 0, 0, IIf(IsDBNull(row("UPRECIO_AFECTOS")), 0, row("UPRECIO_AFECTOS")), IIf(IsDBNull(row("UPRECIO_AFECTOD")), 0, row("UPRECIO_AFECTOD")))
                        Next
                    End If
                Next
                FormMantDocExp.txtproveedor.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("PROVEEDOR").Value
                Dim prov1 As String = PROVISIONESBL.SelectT_Prov(FormMantDocExp.txtproveedor.Text)
                FormMantDocExp.txtnomproveedor.Text = prov1
                FormMantDocExp.txtmskfecgene.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF1").Value, FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF1").Value,
                                                     FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF1").Value)
                FormMantDocExp.txtmskfecprov.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF1").Value, FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF1").Value,
                                                     FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF1").Value)
                FormMantDocExp.txtmoneda.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("MONEDA").Value
                prov1 = PROVISIONESBL.Select_MON_row(FormMantDocExp.txtmoneda.Text)
                FormMantDocExp.txtnom_moneda.Text = prov1
                If FormMantDocExp.txtt_doc.TextLength = 0 And FormMantDocExp.txtser_doc_ref.TextLength = 0 And FormMantDocExp.txtnumero.TextLength = 0 Then
                    FormMantDocExp.txtt_doc.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value
                    FormMantDocExp.cmbt_doc.SelectedValue = FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value
                    FormMantDocExp.txtser_doc_ref.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value
                    FormMantDocExp.txtnumero.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF").Value
                    FormMantDocExp.btnagregar.Enabled = True
                    FormMantDocExp.btnborrar.Enabled = True
                    FormMantDocExp.btndocu.Enabled = True
                    FormMantDocExp.btnmodificar.Enabled = True
                End If
            ElseIf rdbfactrecp.Checked Then
                For i = 0 To lvbusqueda1.Items.Count - 1
                    If lvbusqueda1.Items(i).Checked = True Then
    ...
    Private Sub lvbusqueda1_ItemCheck(sender As Object, e As ItemCheckEventArgs) Handles lvbusqueda1.ItemCheck
        ...
        Dim dt As DataTable
        If FormMantDocExp.dgvt_doc.Rows.Count > 0 Then
            If lvbusqueda1.Items(e.Index).Checked = True Then
                Exit Sub
            Else
                For i = 0 To FormMantDocExp.dgvt_doc.Rows.Count - 1
                    If FormMantDocExp.dgvt_doc.Rows(i).Cells("NRO_DOC_REF").Value = lvbusqueda1.Items(e.Index).SubItems(2).Text Then
        ...
        If gnOpcion3 = "0" Then
            ...
            If rdboreq.Checked Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    Dim fecprov As String = Replace(FormMantDocExp.txtmskfecprov.Text, "/", "")
                    Dim fecprov1 As String = Replace(fecprov, "_", "")
                    Dim fecprov2 As String = Replace(fecprov1, " ", "")
                    'contador = 0
                    Try
                        FormMantDocExp.txtmskfecprov.Text = CDate(FormMantDocExp.txtmskfecprov.Text)
                    Catch ex As Exception
                        ...
                        If fecprov2.Length < 10 And fecprov2.Length > 0 Then
                            ...
                            FormMantDocExp.txtmskfecprov.Focus()
                        End If
                        'Else
                        '    Exit Sub
                        '    'txtmskfecprov.Focus()
                        'End If
                    End Try
                    If Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2) = "1" Then
                        mesfecha = "0" & Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2)
                    Else
                        mesfecha = Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2)
                    End If
                    If Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2) = "1" Then
                        mesdia = "0" & Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2)
                    Else
                        mesdia = Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2)
                    End If
                    dt = REQUERIMIENTOBL.getT_CAMB(Mid(FormMantDocExp.txtmskfecgene.Text, 7, 4) & "/" & mesfecha & "/" & mesdia)
                    For Each Registro In dt.Rows
                        cmb = IIf(IsDBNull(Registro("VENTA")), "", Registro("VENTA"))
                    Next

                    For i = 0 To FormMantDocExp.dgvt_doc.Rows.Count - 1
                        FormMantDocExp.dgvt_doc.Rows(i).Cells("T_CAMB").Value = cmb
                    Next
                    ...
                    FormMantDocExp.dgvt_doc.Rows.Add(FormMantDocExp.txtt_doc.Text, FormMantDocExp.txtser_doc_ref.Text, FormMantDocExp.txtnumero.Text,
                                                         ...
                                                         FormMantDocExp.txtmskfecgene.Text, gsUser, RTrim(Date.Now), IIf(IsDBNull(row("PROVEEDOR")), 0, row("PROVEEDOR")), 0, 0, 0,
                                                         0, 0, 0, 0, 0, 0, IIf(IsDBNull(row("UPRECIO_AFECTOS")), 0, row("UPRECIO_AFECTOS")), IIf(IsDBNull(row("UPRECIO_AFECTOD")), 0, row("UPRECIO_AFECTOD")),
                                                         IIf(IsDBNull(row("NOM_ART")), 0, row("NOM_ART")), IIf(IsDBNull(row("SER_DOC_REF1")), 0, row("SER_DOC_REF1")),
                                                         IIf(IsDBNull(row("NRO_DOC_REF1")), "", row("NRO_DOC_REF1")))
                    If contador = 1 Then
                        FormMantDocExp.txtt_pago.Text = IIf(IsDBNull(row("F_PAGO_ENT")), "", row("F_PAGO_ENT"))
                        FormMantDocExp.cmbt_pago.SelectedValue = IIf(IsDBNull(row("F_PAGO_ENT")), "", row("F_PAGO_ENT"))
            ...
            ElseIf rdbfactrecp.Checked Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    End If
                    If FormMantDocExp.txtmoneda.Text = "00" Then
                        'If Mid(FormMantDocExp.txtmskfecgene.Text) = 0 Then
                        FormMantDocExp.txtmskfecgene.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF1").Value, Mid(FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF1").Value, 1, 3),
                        FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF1").Value)
                        FormMantDocExp.txtmskfecprov.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF1").Value, Mid(FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF1").Value, 1, 3),
                        FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF1").Value)
                        'End If
                        If Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2) = "1" Then
                            mesfecha = "0" & Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2)
                    ...
                    FormMantDocExp.dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")), nro_prov,
                                                     IIf(IsDBNull(row("NRO_DOC_REF")), "", row("NRO_DOC_REF")),
                ...
                If FormMantDocExp.txtt_doc.TextLength = 0 And FormMantDocExp.txtser_doc_ref.TextLength = 0 And FormMantDocExp.txtnumero.TextLength = 0 Then
                    FormMantDocExp.txtt_doc.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value
                    FormMantDocExp.cmbt_doc.SelectedValue = FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value
                    If FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value <> "CLL" Then
                        nro_prov = FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value
                        nro_prov = nro_prov.PadLeft(4, "0")
                    Else
                        nro_prov = FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value
                    End If
                    FormMantDocExp.txtser_doc_ref.Text = nro_prov
                    FormMantDocExp.txtnumero.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF").Value
                    FormMantDocExp.btnagregar.Enabled = True
                    FormMantDocExp.btnborrar.Enabled = True
                    FormMantDocExp.btndocu.Enabled = True
            ...
            ElseIf rdbfactComp.Checked Then
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    End If
                    If FormMantDocExp.txtmoneda.Text = "00" Then
                        'FormMantDocExp.txtmskfecgene.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value, Mid(FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value, 1, 3),
                        'FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF").Value)
                        'FormMantDocExp.txtmskfecprov.Text = PROVISIONESBL.Select_Fecha(FormMantDocExp.dgvt_doc.Rows(0).Cells("T_DOC_REF").Value, Mid(FormMantDocExp.dgvt_doc.Rows(0).Cells("SER_DOC_REF").Value, 1, 3),
                        'FormMantDocExp.dgvt_doc.Rows(0).Cells("NRO_DOC_REF").Value)
                        If Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2) <> "" Then
                            If Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2).Length = "1" Then
                                mesfecha = "0" & Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2).PadLeft(1, "0")
                            Else
                                mesfecha = Mid(FormMantDocExp.txtmskfecgene.Text, 4, 2)
                            End If
                            If Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2).Length = "1" Then
                                mesdia = "0" & Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2)
                            Else
                                mesdia = Mid(FormMantDocExp.txtmskfecgene.Text, 1, 2)
                            End If
                    ...
                    End If
                Next
                FormMantDocExp.txtproveedor.Text = FormMantDocExp.dgvt_doc.Rows(0).Cells("CTCT_COD").Value
                Dim prov1 As String = PROVISIONESBL.SelectT_Prov(FormMantDocExp.txtproveedor.Text)
                FormMantDocExp.txtnomproveedor.Text = prov1
    ...
    Private Sub FormDocuProvExp_Load(sender As Object, e As EventArgs) Handles MyBase.Load
