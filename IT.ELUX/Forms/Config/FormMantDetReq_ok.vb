...
Public Class FormMantDetReq_ok
    ...
    Private Sub btnagregar_Click(sender As Object, e As EventArgs) Handles btnagregar.Click
        Dim activo = txtactivo.Text

        If cmb_tipoReq.SelectedIndex = -1 Then
            Dim dtUsuario As New DataTable
            dtUsuario = ARTICULOBL.verificarUSuario(gsCodUsr)
            If dtUsuario.Rows(0).ItemArray(0) = "113" Then
                MsgBox("Seleccione Tipo de Requerimieto")
                cmb_tipoReq.Focus()
                Exit Sub
            End If
        End If
        ...
        If bFecha >= Format(CDate("21/12/2020"), "dd/MM/yyyy") And txtarticulo.Visible = True Then
            If txtarticulo.Text = Nothing Then
                If MessageBox.Show("Activo no ingresado Desea continuar ?", "Aviso", MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2) = DialogResult.No Then
                    Exit Sub
                End If
            End If
            If npdcantidad.Value > 0 Then
                            ...
                            If FormReque_Ok.dgvt_doc.Rows.Count > 0 Then
                                For Each row As DataRow In dtArticulo.Rows
                                    ...
                                    For i = 0 To FormReque_Ok.dgvt_doc.Rows.Count - 1
                                        ...
                                        If FormReque_Ok.dgvt_doc.Rows(i).Cells(8).Value = Mid(IIf(IsDBNull(row("CODIGO")), "", row("CODIGO")), 1, 8) Then

                                            FormReque_Ok.dgvt_doc.Rows(i).Cells(7).Value = FormReque_Ok.dgvt_doc.Rows(i).Cells(7).Value + (npdcantidad.Value * IIf(IsDBNull(row("CANTIDAD")), 1, row("CANTIDAD")))
                                            x = "1"

                                            '     For i = 0 To dtArticulo.Rows.Count - 1
                                    ...
                                    If x = "0" Then
                                        FormReque_Ok.dgvt_doc.Rows.Add(FormReque_Ok.txtt_doc.Text, '0
                                                                                  ...
                                                                                  FormReque_Ok.txtproveedor.Text,'6 
                                                                                  (npdcantidad.Value * IIf(IsDBNull(row("CANTIDAD")), 1, row("CANTIDAD"))), '7
                                                                                  Mid(IIf(IsDBNull(row("CODIGO")), "", row("CODIGO")), 1, 8),'8
                                                                                  Mid(IIf(IsDBNull(row("CODIGO")), "", row("CODIGO")), 12), '9
                                                                                  Nothing, '10
                                                                                  "",'11
                                                                                  "", "", "", "+", txtobservacion.Text, FormReque_Ok.txtt_movinv.Text, "", "", "", "", "", "",
                                                                                  "", "", "", FormReque_Ok.dtpfecha.Text, gsUser, cmbuni.SelectedValue, FormReque_Ok.txtt_pago.Text, FormReque_Ok.txtfor_ent.Text,
                ...
                ElseIf gContador = 0 Then
                    ...
                    FormReque_Ok.dgvt_doc.Rows(FormReque_Ok.dgvt_doc.CurrentRow.Index).Cells(48).Value = cmb_tipoReq.SelectedItem
    ...
    Private Sub npdcantidad_ValueChanged(sender As Object, e As EventArgs) Handles npdcantidad.ValueChanged
        Dim cant = npdcantidad.Value
        npdpeso.Value = cant
    End Sub
End Class