...
#End Region
    ...
    Private Sub FormMantFCRecepComp_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dgvt_doc.Columns.Add("ART_PRECIO", "ART_PRECIO") '19 CANTIDAD2
        dgvt_doc.Columns.Add("ART_PRECIOPROM", "ART_PRECIOPROM") '20 CANTIDAD5
        dgvt_doc.Columns.Add("ART_PRECIOPROMN", "ART_PRECIOPROMN") '21 CANTIDAD4
        dgvt_doc.Columns.Add("PER_COD", "PER_COD") '22
        dgvt_doc.Columns.Add("SREQ", "SREQ") '23
        dgvt_doc.Columns.Add("NREQ", "NREQ") '24
        dgvt_doc.Columns.Add("UNIDAD", "UNIDAD") '25
        dgvt_doc.Columns.Add("ART_VENTA", "ART_VENTA") '26  'New
        dgvt_doc.Columns.Add("EST", "UNIDAD") '27
        dgvt_doc.Columns.Add("CT", "CT") '28
        dgvt_doc.Columns.Add("NRO_DOCU2", "NRO_DOCU2") '29
        dgvt_doc.Columns.Add("NRO_DOCU3", "NRO_DOCU3") '30
        dgvt_doc.Columns.Add("SER_DOCU3", "SER_DOCU3") '31
        dgvt_doc.Columns.Add("SER_REQ", "SER_REQ") '32
        dgvt_doc.Columns.Add("NRO_REQ", "NRO_REQ") '33
        dgvt_doc.Columns.Add("ART_VENTA1", "ART_VENTA1") '34
        ...
        Select Case gnOpcion
            Case 0
                ...
                txtnro_doc_ref.Text = nro.PadLeft(7, "0")
            ...
            Case 1
                ...
                For Each row As DataRow In dtArticulo.Rows
                    dgvt_doc.Rows.Add(IIf(IsDBNull(row("T_DOC_REF")), "", row("T_DOC_REF")),'0
                                      ...
                                      IIf(IsDBNull(row("ART_PRECIO")), 0, row("ART_PRECIO")),'29
                                      IIf(IsDBNull(row("ART_PRECIOPROM")), 0, row("ART_PRECIOPROM")),'30
                                      IIf(IsDBNull(row("ART_PRECIOPROMN")), "", row("ART_PRECIOPROMN")),'31
                                      ...
                                      "", "", IIf(IsDBNull(row("ART_VENTA1")), "", row("ART_VENTA1"))) '34
                ...
                Try
                    ...
                    For Each item As Object In cmbalmacen.Items
                        Dim value As String = item.ToString()
                        Dim parts() As String = value.Split("-"c)
                        If parts.Length = 2 AndAlso parts(0) = GUIAALMACENBL.SelectAlm("SALM", "ALM", s) Then
                            cmbalmacen.SelectedItem = item
                            Exit For
                        End If
                    Next
    ...
    Private Sub SaveData()
        ...
        If flagAccion = "N" Then
            ...
            If m1 <> dgvt_doc.Rows.Count Then
                ...
                GUIAALMACENBE.NRO_DOC_REF = nro.PadLeft(7, "0")
                ELTBDETRECEPCOMPBE.NRO_DOCU2 = nro.PadLeft(7, "0")
        ...
        GUIAALMACENBE.ALM_COD = Mid(cmbalmacen.Text, 1, 4)
        ...
        If gsError = "OK" Then
            ...
            If flagAccion = "N" Then
                ...
                If ca <> dgvt_doc.Rows.Count Then
                    'If gsUser = "SISTEMA" Then
                    gsError2 = ELTBRECEPCOMPBL.SaveRowGuia(GUIAALMACENBE, DET_DOCUMENTOBE, ELMVALMABE, ELMVLOGSBE, "CNR", dgvt_doc, cmb_serdoc.Text, sEstAlmac)
                    'Else
            ...
            Else
                ...
                If ca <> dgvt_doc.Rows.Count Then
                    ...
                    GUIAALMACENBE.SER_DOC_REF = "ALM"
                    ' If gsUser = "SISTEMA" Then
                    gsError2 = ELTBRECEPCOMPBL.SaveRowGuia(GUIAALMACENBE, DET_DOCUMENTOBE, ELMVALMABE, ELMVLOGSBE, "CMR", dgvt_doc, cmb_serdoc.Text, sEstAlmac)
                    'Else
    ...
    Private Sub btnRotulo_Click(sender As Object, e As EventArgs) Handles btnRotulo.Click
        ...
        If dgvt_doc.Rows.Count > 0 Then
            ...
            For i = 0 To dgvt_doc.Rows.Count - 1
                ...
                frm.dtpFecIngreso.Value = dtpfec_gene.Value
                frm.txtCantGuia.Text = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(13).Value
                numReq = ELTBRECEPCOMPBL.SelReqOreq("001", dgvt_doc.Rows(i).Cells("NRO_DOC_REF1").Value, dgvt_doc.Rows(i).Cells("ART_COD").Value) 'dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(5).Value
                artCodReq = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(14).Value
                tipGuia = "SALM"
                serGuia = "ALM"
                If dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(26).Value = "" Then
                    numGUia = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(34).Value
                Else
                    numGUia = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells(26).Value
                End If
                frm.ShowDialog()
            Next
