...
Public Class FormDocuRefConta
    ...
    Private Sub btnbuscar_Click(sender As Object, e As EventArgs) Handles btnbuscar.Click
        ...
        Dim s As String = Mid(FormELPAGO_DOCUMENTO.cmbt_ope.Text, 1, 3)
        ...
        If dt.Rows.Count > 0 Then
            For Each row As DataRow In dt.Rows
                ...
                item.SubItems.Add(IIf(IsDBNull(row("TIPO7")), "", row("TIPO7")))
                item.SubItems.Add(IIf(IsDBNull(row("FLUJO")), "", row("FLUJO")))
                item.SubItems.Add(IIf(IsDBNull(row("EST_MERCADERIA")), "", row("EST_MERCADERIA")))
                item.SubItems.Add(IIf(IsDBNull(row("CAJA")), "", row("CAJA")))
                item.SubItems.Add(IIf(IsDBNull(row("T_CMB")), "", row("T_CMB")))
                item.SubItems.Add(IIf(IsDBNull(row("T_OPE_COD")), "", row("T_OPE_COD")))
            Next
        ...
        If lvbusqueda1.Items.Count > 0 Then
            txtRegistros.Text = lvbusqueda1.Items.Count
        End If
        ...
        Else
            ...
            For i = 0 To lvbusqueda1.Items.Count - 1
                If lvbusqueda1.Items(i).Checked = True Then
                    ...
                    If lvbusqueda1.Items(i).SubItems(10).Text = "01" Then

                        t_cambio = lvbusqueda1.Items(i).SubItems(7).Text / lvbusqueda1.Items(i).SubItems(8).Text
                        t_cambio = Math.Round(t_cambio, 3)
                    End If

                    Dim signo = ""
                    Select Case lvbusqueda1.Items(i).SubItems(0).Text
                        Case "80", "01", "03", "14", "00", "08", "91", "21", "36"
                            If gsOperacion = "010" Then
                                signo = "+"
                            Else
                                signo = "-"
                            End If
                    ...
                    End Select
                    Dim dolares = 0.00
                    If lvbusqueda1.Items(i).SubItems(8).Text = 0 Then
                        'actualizado 12-10-2022
                        If Label9.Text = "1" Then
                            dolares = Format(lvbusqueda1.Items(i).SubItems(7).Text / lvbusqueda1.Items(i).SubItems(16).Text, "#.#0")
                        Else
                            If t_cambio = 0 Then
                                dolares = Format(lvbusqueda1.Items(i).SubItems(7).Text / lvbusqueda1.Items(i).SubItems(16).Text, "#.#0")
                            Else
                                dolares = Format(lvbusqueda1.Items(i).SubItems(7).Text / t_cambio, "#.#0")
                            End If

                        End If
                    Else
                        dolares = lvbusqueda1.Items(i).SubItems(8).Text
                    End If
                    Dim codflujo = ""
                    Dim flujo = ""
                    Dim codCaja = ""
                    Dim caja = ""
                    If (gsUser = "SGARCIA" Or gsUser = "OBEIZAGA") And (lvbusqueda1.Items(i).SubItems(17).Text = "001" Or lvbusqueda1.Items(i).SubItems(17).Text = "008" Or
                        lvbusqueda1.Items(i).SubItems(17).Text = "007") Then
                        If lvbusqueda1.Items(i).SubItems(12).Text = "" Then
                            codflujo = "1"
                            flujo = "ACTIVIDADES DE OPERACION"
                            codCaja = "01"
                            caja = "VENTA DE VIENES"
                        Else
                            codflujo = lvbusqueda1.Items(i).SubItems(12).Text
                            flujo = lvbusqueda1.Items(i).SubItems(13).Text
                            codCaja = lvbusqueda1.Items(i).SubItems(14).Text
                            caja = lvbusqueda1.Items(i).SubItems(15).Text
                        End If
                    Else
                        codflujo = lvbusqueda1.Items(i).SubItems(12).Text
                        flujo = lvbusqueda1.Items(i).SubItems(13).Text
                    ...
                    If Label9.Text = "1" Then
                        'actualizado 12-10-2022
                        FormELPAGO_DOCUMENTO.dgvt_doc.Rows.Add(lvbusqueda1.Items(i).SubItems(0).Text,
                                                           lvbusqueda1.Items(i).SubItems(1).Text,
                                                           lvbusqueda1.Items(i).SubItems(2).Text,
                                                           "NO",
                                                           lvbusqueda1.Items(i).SubItems(9).Text,
                                                           "",
                                                           lvbusqueda1.Items(i).SubItems(4).Text,
                                                           lvbusqueda1.Items(i).SubItems(5).Text,
                                                           signo,
                                                           fechadia,
                                                           lvbusqueda1.Items(i).SubItems(16).Text,
                                                           lvbusqueda1.Items(i).SubItems(10).Text,
                                                           lvbusqueda1.Items(i).SubItems(11).Text,
                                                           lvbusqueda1.Items(i).SubItems(7).Text,
                                                           dolares,
                                                           "",
                                                           codflujo, flujo, codCaja, caja, False, "1")
                    Else
                        If t_cambio = 0 Then
                            FormELPAGO_DOCUMENTO.dgvt_doc.Rows.Add(lvbusqueda1.Items(i).SubItems(0).Text,
                                                           lvbusqueda1.Items(i).SubItems(1).Text,
