...
#End Region
    ...
    Private Sub SaveData()
        ...
        LETRASBE.PORCENTAJE = Val(txtalm_cod.Text)
        If chkcod_color.Checked Then
            LETRASBE.COD_COLOR = "1"
        End If
        If chkcod_medida.Checked Then
            LETRASBE.COD_MEDIDA = "1"
        End If
        ...
        Try
            ...
            Select Case sFunc
                ...
                Case "exit"
                    ...
                    Exit Sub
                Case "Print"
                    Dim RESP = MsgBox("Desea Imprimir Letra con Diseño", MsgBoxStyle.YesNo)
                    If RESP = 6 Then
                        ReDim gsRptArgs(2)
                        gsRptArgs(0) = txtt_doc.Text
                        gsRptArgs(1) = cmb_serdoc.SelectedItem
                        gsRptArgs(2) = txtnumero.Text
                        gsPathRpt = gsIpserver & "sistema\E.ELUX\REPORTES\02\RPT02_DOCUMENTO_LETRA_DISEÑO.rpt"
                        gsRptPath = gsPathRpt
    ...
    Private Sub GetData(ByVal sT_Ref As String, ByVal sS_Ref As String, ByVal sN_Ref As String)
        ...
        For Each Registro In dtUsuario.Rows
            ...
            If IIf(IsDBNull(Registro("FARDO1")), "", Registro("FARDO1")).ToString.Length > 0 Then
                chkcod_color.Checked = True
            End If
            If IIf(IsDBNull(Registro("FARDO2")), "", Registro("FARDO2")).ToString.Length > 0 Then
                chkcod_medida.Checked = True
            End If
            ...
            txtalm_cod.Text = IIf(IsDBNull(Registro("PORCENTAJE")), "", Registro("PORCENTAJE"))
...
#Region "Txt"
    ...
    '        ElseIf txtmon.Text = "01" Then
    '            If txtmontopagadod.TextLength <> 0 Then
    '                txtmontopagadod.Text = (txtmontopagadod.Text * txtalm_cod.Text / 100) + txtmontopagadod.Text
    '            End If
    '        End If
    '    End If
    'End Sub
