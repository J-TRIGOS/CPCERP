...
Public Class FormMantDetLet_Monto
    ...
    Private Function OkData() As Boolean
        ...
        For i As Integer = 0 To dgvt_doc.Rows.Count - 1
            If dgvt_doc.Rows(i).Cells("F_PAGO_ENT").Value = "" Then
                MsgBox("No se esta guardando el tipo de pago", MsgBoxStyle.Exclamation)
            End If
        Next
        Return True
    End Function
    ...
    Private Sub FormMantDetLet_Monto_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        dgvt_doc.Columns.Add("PERCEPCION", "PERCEPCION") '22
    ...
    Private Sub btnagregar_Click(sender As Object, e As EventArgs) Handles btnagregar.Click
        ...
        If dgvt_doc.Rows.Count > 0 Then
            ...
            ElseIf txtmoneda.Text = "01" Then
                ...
                If Math.Round(acum, 2) > npdmontod_fact.Value Then
                    MsgBox("El monto ya supero el monto de la factura Corrija")
                    Exit Sub
                End If
            End If
        ...
        dgvt_doc.Rows.Add("", "", "",
                          ...
                          "H", "", "", "", "", "", txtpercepcion.Text)
        npdmontod.Value = 0
        txtt_pago.Text = ""
        cmbt_pago.SelectedIndex = -1

        txtpercepcion.Text = Nothing
        txtcantper.Text = Nothing

        For i As Integer = 0 To dgvt_doc.Rows.Count - 1
            If dgvt_doc.Rows(i).Cells("F_PAGO_ENT").Value = "" Then
                MsgBox("No se esta guardando el tipo de pago" + txtt_pago.Text, MsgBoxStyle.Exclamation)
            End If
        Next
                ...
                Case "Print"
                    If dgvt_doc.Rows.Count > 0 Then
                        For i = 0 To dgvt_doc.Rows.Count - 1
                            ...
                            gsPathRpt = gsIpserver & "sistema\E.ELUX\REPORTES\02\RPT02_DOCUMENTO_LETRA.rpt"
                            gsRptPath = gsPathRpt
    ...
    Private Sub txtpercepcion_LostFocus(sender As Object, e As EventArgs) Handles txtpercepcion.LostFocus
        If txtmoneda.Text = "00" Then
            txtcantper.Text = (npdmontos.Value * Val(txtpercepcion.Text) / 100) + npdmontos.Value
        ElseIf txtmoneda.Text = "01" Then
            txtcantper.Text = (npdmontod.Value * Val(txtpercepcion.Text) / 100) + npdmontod.Value
        End If
    End Sub

    Private Sub txtpercepcion_TextChanged(sender As Object, e As EventArgs) Handles txtpercepcion.TextChanged
        '   If txtmoneda.Text = "00" Then
        '       txtcantper.Text = (npdmontos.Value * Val(txtpercepcion.Text) / 100) + npdmontos.Value
        '   ElseIf txtmoneda.Text = "01" Then
        '       txtcantper.Text = (npdmontod.Value * Val(txtpercepcion.Text) / 100) + npdmontod.Value
    ...
    Private Sub npdmontos_LostFocus(sender As Object, e As EventArgs) Handles npdmontos.LostFocus
        If GroupBox1.Enabled = True Then
            txtcantper.Text = (npdmontos.Value * Val(txtpercepcion.Text) / 100) + npdmontos.Value
        End If
    End Sub

    Private Sub npdmontod_LostFocus(sender As Object, e As EventArgs) Handles npdmontod.LostFocus
        txtcantper.Text = (npdmontod.Value * Val(txtpercepcion.Text) / 100) + npdmontod.Value
    End Sub


    Private Sub dgvt_doc_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvt_doc.CellClick
        If dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("PERCEPCION").Value <> Nothing Then
            txtpercepcion.Text = dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("PERCEPCION").Value
            If dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONEDA").Value = "00" Then
                txtcantper.Text = (Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONTOS").Value) * Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("PERCEPCION").Value) / 100) + Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONTOS").Value)
            ElseIf dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONEDA").Value = "01" Then
                txtcantper.Text = (Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONTOD").Value) * Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("PERCEPCION").Value) / 100) + Val(dgvt_doc.Rows(dgvt_doc.CurrentRow.Index).Cells("MONTOD").Value)
            End If
        End If
    End Sub
End Class