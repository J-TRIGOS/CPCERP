...
Imports IT.ELUX.BE
Imports IT.ELUX.BL
Public Class FormMantNotaDebito
...
#End Region
    ...
    Private Sub SaveData()
        ...
        If flagAccion = "N" Then
            nro = NOTADEBITOBL.SelectNro1(txtt_doc.Text, cmb_serdoc.SelectedItem)
        ...
        NOTADEBITOBE.T_DOC_REF = RTrim(txtt_doc.Text)
        NOTADEBITOBE.SER_DOC_REF = RTrim(cmb_serdoc.Text)
        NOTADEBITOBE.NRO_DOC_REF = RTrim(txtnumero.Text)
        NOTADEBITOBE.ART_COD = "07010003"
        NOTADEBITOBE.FEC_GENE = RTrim(dtpfecha.Value)
        ...
        If dtpfanul.Checked Then
            NOTADEBITOBE.FEC_ANU = dtpfanul.Value
        Else
            NOTADEBITOBE.FEC_ANU = Nothing
        End If
        NOTADEBITOBE.SIGNO = "-"
        NOTADEBITOBE.OBSERVA = RTrim(txtobservacion.Text)
        NOTADEBITOBE.MONEDA = txtmon.Text
        NOTADEBITOBE.DIR_COD = RTrim(txtdir.Text)
        ...
        Next
        NOTADEBITOBE.TPRECIO_VENTA = DAcumula
        NOTADEBITOBE.TPRECIO_DVENTA = DAcumula1
        NOTADEBITOBE.T_DCTO = DAcumula2
        ...
        NOTADEBITOBE.NUMPEDIDO = txtoc.Text
        NOTADEBITOBE.USUARIO = RTrim(gsUser)
        NOTADEBITOBE.F_PAGO_ENT = RTrim(txtt_pago.Text)
        NOTADEBITOBE.FOR_ENT_COD = RTrim(txtfor_ent.Text)
        NOTADEBITOBE.T_MOVINV = txtt_movinv.Text
        NOTADEBITOBE.VENDEDOR = txtvendedor.Text
        ELMVLOGSBE.log_codusu = gsCodUsr
        'NOTADEBITOBE.REG_NRO = Replace(Mid(dtpfecha.Value, 4, 7), "/", "")
        NOTADEBITOBE.MOT_COD = txtmotcod.Text
        mesaño = Replace(Mid(dtpfecha.Value, 4, 7), "/", "")
        m = Mid(dtpfecha.Value, 4, 2)
        gsError = NOTADEBITOBL.SaveRow(NOTADEBITOBE, DET_DOCUMENTOBE, ELMVLOGSBE, flagAccion, dgvt_doc)
        If gsError = "OK" Then
            MsgBox("Datos Grabados Correctamente", MsgBoxStyle.Information)
            'gsError2 = NOTADEBITOBL.AsientoFC("Asiento", mesaño, m, txtnumero.Text, cmb_serdoc.Text, txtt_doc.Text, NOTADEBITOBE.EST, dgvt_doc)
            'If gsError2 = "OK" Then
    ...
    Private Sub GetData(ByVal sT_Ref As String, ByVal sS_Ref As String, ByVal sN_Ref As String)
        ...
        'MODIFICAR
        dtUsuario = NOTADEBITOBL.SelectRow(sT_Ref, sS_Ref, sN_Ref)
        ...
        For Each Registro In dtUsuario.Rows
            ...
            Dim dt As DataTable
            dt = NOTADEBITOBL.SelectDir(txtctct_cod.Text)
            Try
        ...
        Try
            ...
            Select Case sFunc
                ...
                Case "Print"
                    ...
                    gsPathRpt = gsIpserver & "sistema\E.ELUX\REPORTES\02\RPT02_IMPRESION_ND.rpt"
                    gsRptPath = gsPathRpt
                ...
                Case "anular"
                    ...
                    Dim NOTADEBITOBE As New NOTADEBITOBE
                    Dim DET_DOCUMENTOBE As New DET_DOCUMENTOBE
                    NOTADEBITOBE.T_DOC_REF = txtt_doc.Text
                    NOTADEBITOBE.SER_DOC_REF = cmb_serdoc.Text
                    NOTADEBITOBE.NRO_DOC_REF = txtnumero.Text


                    Dim ELMVLOGSBE As New ELMVLOGSBE
                    ELMVLOGSBE.log_codusu = gsCodUsr
                    gsError = NOTADEBITOBL.SaveRow(NOTADEBITOBE, DET_DOCUMENTOBE, ELMVLOGSBE, "AR", dgvt_doc)
    ...
    End Sub
    Private Sub FormMantNotaDEBITO_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        txtt_doc.Select()
        bPrimero = True
        gsError = ""
        gpCaption = "Nota Debito"
        Me.Text = gpCaption
        ...
        'Cargar los combos
        Dim dt As DataTable = NOTADEBITOBL.SelectT_DOC("08")
        GetCmb("cod", "nom", dt, cmbt_doc)
        dt = NOTADEBITOBL.SelectT_MOVINV()
        GetCmb("cod", "nom", dt, cmbt_movinv)
        dt = NOTADEBITOBL.SelectF_PAGO
        GetCmb("cod", "nom", dt, cmbt_pago)
        dt = NOTADEBITOBL.SelectF_ENT
        GetCmb("cod", "nom", dt, cmbfor_ent)
        dt = NOTADEBITOBL.SelectMoneda
        GetCmb("cod", "nom", dt, cmbmon)
        cmbmon.SelectedIndex = 1
        txtmon.Text = "01"
        dt = NOTADEBITOBL.SelectVendedor
        GetCmb("cod", "nom", dt, cmbvendedor)
        dt = NOTADEBITOBL.SelectCliente
        ...
        Select Case gnOpcion
            ...
            Case 1
                flagAccion = "M"
                GetData("08", sSDoc, sNDoc)
                bCuarto = "1"

                'habilitar(True)
                'cmb_serdoc.Enabled = False
                Dim dtArticulo As DataTable
                dtArticulo = NOTADEBITOBL.getListdgv("08", sSDoc, sNDoc)
    ...
    Private Sub FormMantNotaDEBITO_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        Dispose()
    End Sub
    ...
    Private Sub btnagregar_Click(sender As Object, e As EventArgs) Handles btnagregar.Click
        ...
        Dim frm As New FormMantDetNotaDebito
